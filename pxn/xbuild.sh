...
PWD=`pwd`
if [[ "${PWD}" == "/usr/"* ]]; then
	echo 'Cannot run xbuild.sh script from this location.' >&2
	exit 1
fi
...
if [ ! -f "${CONFIG_FILE}" ]; then
	newline
	errcho 'xbuild.conf file not found here!'
...
CheckConfigVersion() {
	if [[ -z $CONFIG_VERSION ]] || [[ $CONFIG_VERSION -ne $REQUIRED_CONFIG_VERSION ]]; then
		errcho 'xbuild.conf is outdated!'
		errcho '  config version: ${CONFIG_VERSION}'
		errcho '  required version: ${REQUIRED_CONFIG_VERSION}'
		exit 1
	fi
}
...
sedVersion() {
	...
	sed -i.original "s@x-SNAPSHOT@${BUILD_NUMBER}-SNAPSHOT@" "${FILENAME}" || {
		errcho "Failed to sed a file! ${FILENAME}"
		restoreSed "${FILENAME}"
		return 1
	}
...
restoreSed() {
	...
	if [ -f "${FILENAME}.original" ]; then
		...
		mv -fv "${FILENAME}.original" "${FILENAME}" || {
			errcho "Failed to restore a sed file! ${FILENAME}"
			return 1
		};
	fi
	return 0
...
BuildMVN() {
	...
	which mvn >/dev/null || {
		BUILD_FAILED=true
		errcho "Maven not installed - yum install maven"
		return 1
	}
	local MVN_GOALS=''
	local MVN_POM=''
	...
	if [ ! -f "${MVN_POM}" ]; then
		BUILD_FAILED=true
		errcho "Pom file ${MVN_POM} not found!"
		return 1
	fi
	...
	mvn ${MVN_GOALS} || { \
		BUILD_FAILED=true
		errcho 'Failed to build maven project!'
		return 1
	}
	newline
	newline
...
BuildComposer() {
	...
	which composer >/dev/null || {
		BUILD_FAILED=true
		errcho 'Composer not installed - yum install php-composer'
		return 1
	}
	# default to /
	if [[ $# -eq 0 ]]; then
	...
	for DIR in "${@}"; do
		...
		if [ ! -d "${PWD}/${DIR}" ]; then
			BUILD_FAILED=true
			errcho "Composer workspace not found: ${DIR}"
			return 1
		fi
		if [ ! -f "${PWD}/${DIR}/composer.json" ]; then
			BUILD_FAILED=true
			errcho "composer.json file not found in workspace: ${DIR}"
			return 1
		fi
		# composer install
		${BIN_PHP} /usr/bin/composer install -v --working-dir "${PWD}/${DIR}" || {
			BUILD_FAILED=true
			errcho "Failed to install with composer: ${DIR}"
			return 1
		}
		RESULT_INSTALLS+=("${DIR}")
...
BuildPhar() {
	...
	which box >/dev/null || {
		BUILD_FAILED=true
		errcho 'Box not installed - yum install php-composer'
		return 1
	}
	# default to /
	if [[ $# -eq 0 ]]; then
	...
	for DIR in "${@}"; do
		...
		if [ ! -d "${PWD}/${DIR}" ]; then
			BUILD_FAILED=true
			errcho "Box workspace not found: ${DIR}"
			return 1
		fi
		if [ ! -f "${PWD}/${DIR}/box.json" ]; then
			BUILD_FAILED=true
			errcho "box.json file not found in workspace: ${DIR}"
			return 1
		fi
		# composer install
		pushd "${PWD}/${DIR}"
		${BIN_PHP} /usr/bin/box build -v || {
			BUILD_FAILED=true
			errcho "Failed to build .phar with box: ${DIR}"
			return 1
		}
		popd
		RESULT_INSTALLS+=("${DIR}")
...
BuildRPM() {
	...
	which rpmbuild >/dev/null || {
		BUILD_FAILED=true
		errcho 'rpmbuild not installed - yum install rpmdevtools'
		return 1
	}
	local RPM_SPEC=''
	local RPM_SOURCES=()
	...
	if [ ! -f "${RPM_SPEC}" ]; then
		BUILD_FAILED=true
		errcho "Spec file ${RPM_SPEC} not found"
		return 1
	fi
...
DeployFiles() {
	...
	if [ $LS_FAIL == true ] || [ $LS_FOUND == false ]; then
		newline
		BUILD_FAILED=true
		errcho 'One or more build result files are missing!'
		return 1
	fi
	unset LS_FAIL
	unset LS_FOUND
	...
	if [ -z $XBUILD_PATH_DOWNLOADS ]; then
		BUILD_FAILED=true
		errcho 'XBUILD_PATH_DOWNLOADS not set in xbuild-deploy.conf'
		return 1
	fi
...
BuildFinished() {
	...
	if [ $BUILD_FAILED != false ]; then
		errcho "Build Failed!  ${BUILD_NAME} ${BUILD_VERSION}"
		exit 1
	fi
